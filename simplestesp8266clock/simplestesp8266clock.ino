...
#define NTP_ADDRESS  "in.pool.ntp.org"  // change this to whatever pool is closest (see ntp.org)
...
SSD1306  display(0x3c, 5, 4); //0x3d for the Adafruit 1.3" OLED, 0x3C being the usual address of the OLED
...
void setup () 
{
  ...
  Wire.pins(5, 4);  // Start the OLED with GPIO 0 and 2 on ESP-01
  Wire.begin(5, 4); // 0=sda, 2=scl
  display.init();
...
void loop() 
{
  ...
  {   
    ...
    TimeChangeRule IST = {"IST", First, Mon, Jan, 0, 270};   //UTC - 6 hours - change this as needed
    Timezone inST(IST, IST);
    local = inST.toLocal(utc);
